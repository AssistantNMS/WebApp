...
export const displaySectionTextItem = (sectionItem: GuideSectionItem, index: number) => {
  return (
    <div key={`${sectionItem.type}-${index}`} className="col-xl-7 col-lg-10 col-md-12 col-sm-12 col-xs-12 item">
...
export const displaySectionLinkItem = (sectionItem: GuideSectionItem, index: number) => {
  return (
    <div key={`${sectionItem.type}-${index}`} className="col-xl-7 col-lg-10 col-md-12 col-sm-12 col-xs-12 item minHeight">
...
export const displaySectionMarkdownItem = (sectionItem: GuideSectionItem, index: number) => {
  return (
    <div key={`${sectionItem.type}-${index}`} className="col-xl-7 col-lg-10 col-md-12 col-sm-12 col-xs-12 item ta-left">
      <Markdown markdown={sectionItem.content} />
    </div>
  );
};
      ...
      <table className="table table-bordered">
        ...
        <tbody>
          {sectionItem.rows.map((rows: string[], index: number) => {
            return (
              <tr key={`row-${rows.length}-${index}`}>
                {rows.map((row: string) => {
                  return <td key={`row-column-${row}`}>{row}</td>;
                })}
              </tr>
            );
          })}
