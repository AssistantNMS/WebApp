...
export const WeekendMissionPresenter: React.FC<IProps> = (props: IProps) => {
  ...
  const handleLoadingOrError = (displayFunc: (props: IProps) => ReactNode): ReactNode => {
    ...
    if (props.status === NetworkState.Error || !props.weekendMissionStage || !props.weekendMissionStage.AppId) {
      return <Error />;
    }
    ...
    return (
      <>
        <div className="row">
          ...
          <div className="col-12 pt1">{weekendMission.NpcMessage}</div>
          <div className="col-12 pt1">
            <PositiveButton
              ...
              onClick={() => {
                ...
                setMessageFlow({
                  ...weekendMission.NpcMessageFlows,
                  timeStamp,
                });
                setOpen(true);
              }}
        ...
        <hr />
        <div className="row">
          <div className="col-12">{translate(LocaleKey.requiresTheFollowing)}</div>
          <div className="col-12">
            <GenericListPresenter list={weekendMissionRequirements} presenter={RequiredItemListTile} isCentered={true} />
          </div>
          <div className="col-12 pt1">
            <PositiveButton
              icon="chat"
        ...
        <div className="row justify noselect">
          {props.level != null && hasPrev && (
            <div className="col-6">
              <PositiveButton
                additionalClass={hasNext ? 'right' : ''}
                icon="arrow_back_ios"
                iconPosition="left"
                onClick={() => {
                  props.navigateToWeekendMissionLevel((props.level ?? 0) - 1);
                }}
              >
