...
export const CommunityMissionPresenter: React.FC<IProps> = (props: IProps) => {
  const handleLoadingOrError = (displayFunc: (props: IProps) => ReactNode): ReactNode => {
    ...
    if (props.status === NetworkState.Error || !props.communityMission || !props.communityMission.missionId) {
      return <Error />;
    }
      ...
      <>
        <div className="row noselect">
          <div className="col-12">
            <h3>
              {translate(LocaleKey.researchProgress)}
              <br />
        ...
        <div className="row noselect">
          <div className="col-12">
            {translate(LocaleKey.communityMissionContent)
              .split('\n')
              .map((text: string, index: number) => (
                <h3 key={`communityMissionContent-${index}`}>{text}</h3>
              ))}
          </div>
        </div>
        ...
        {displayQuicksilverData(
          {
            ...communityMission,
            missionId: communityMission.missionId + 1,
            currentTier: 0,
          },
          quicksilverRewards,
        ...
        <div className="row justify noselect">
          <Link to={communityMissionTimeline} draggable={false} className="customButton center full mt-3em mh-1em noselect">
  ...
  const displayQuicksilverData = (communityMission: CommunityMissionViewModel, quicksilverRewards: Array<QuicksilverStore>, title?: LocaleKey) => {
    ...
    const customQuicksilverItemListTile = (communityMission: CommunityMissionViewModel) => (qProps: QuicksilverItem) => {
      let isDisabled = qProps.Tier >= communityMission.currentTier;
      if (qProps.Tier === communityMission.currentTier && communityMission.percentage > 99) {
        isDisabled = false;
      }
    ...
    const customRequiredItemListTile = (appId: string) => {
    ...
    let localTitle: string | undefined = title !== null ? translate(title!) : title;
          ...
          <div className="row noselect">
            ...
            <div className="col-12 pb-3">
              <GenericListPresenter
                ...
                presenter={customRequiredItemListTile}
    ...
    <DefaultAnimation>
      ...
      <div className="content">
        <div className="container full pt1 pb5">
          {handleLoadingOrError((localProps: IProps) => displayCommunityMissionData(localProps.communityMission, localProps.quicksilverStoreItems))}
        </div>
      </div>
    </DefaultAnimation>
  );
