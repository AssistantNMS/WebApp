...
import {
  ExpeditionSeason,
  ExpeditionSeasonMilestone,
  ExpeditionSeasonPhase,
...
export const CurrentExpeditionSeasonHeader: React.FC<ICurrentExpeditionSeasonHeaderProps> = (props: ICurrentExpeditionSeasonHeaderProps) => {
  if (props.networkState === NetworkState.Loading)
    return (
      <>
        <SmallLoading maxWidth="50px" hideText={true} />
        <hr />
      </>
    );

  if (props.networkState === NetworkState.Error || props.seasonDetails == null) return <span></span>;
...
export const ExpeditionSeasonHeader: React.FC<IExpeditionSeasonHeaderProps> = (props: IExpeditionSeasonHeaderProps) => {
  ...
  if (props.networkState === NetworkState.Error || props.seasonDetails == null) return <Error />;
  ...
  return (
    <div data-id="ExpeditionSeasonHeader" className="row expedition-season-header">
      <div className="col-12 col-lg-3 col-md-3 col-sm-12 col-xs-3 noselect">
        <LazyLoadImage
          src={`/${AppImage.base}${props.seasonDetails.Icon}`}
          alt={props.seasonDetails.Title}
          style={{ width: '100%', maxWidth: '250px' }}
        />
        <h4>
          <b>{translate(LocaleKey.startDate)}:</b>&nbsp;
          {formatDate(props.seasonDetails.StartDate, 'YYYY-MM-DD')}
        </h4>
        <h4>
          <b>{translate(LocaleKey.endDate)}:</b>&nbsp;
          {formatDate(props.seasonDetails.EndDate, 'YYYY-MM-DD')}
        </h4>
      </div>
...
export const ExpeditionSeasonPhases: React.FC<IExpeditionSeasonPhaseProps> = (props: IExpeditionSeasonPhaseProps) => {
  ...
  if (props.networkState === NetworkState.Error || props.phases == null) return <Error />;
    ...
    <div data-id="ExpeditionSeasonPhases" className="row expedition-season-phases">
      <div className="col-12">
        {props.phases.map((phase: ExpeditionSeasonPhase, index: number) => (
          <ExpeditionSeasonPhaseWithMilestones key={`${phase?.Id}-${index}`} phase={phase} setDetailPane={props.setDetailPane} />
        ))}
      </div>
    </div>
  );
...
const rewardsIconPresenter = (icon: string, title: string) => {
  return <img src={`/assets/images/${icon}`} className="hidden-in-mobile milestone-patch" alt={title} draggable={false} />;
};
...
export const ExpeditionSeasonPhaseWithMilestones: React.FC<IExpeditionSeasonPhaseWithMilestonesProps> = (
  props: IExpeditionSeasonPhaseWithMilestonesProps,
) => {
  ...
  return (
    <div data-id="ExpeditionSeasonPhaseWithMilestones" className="row expedition-season-phase noselect">
      <div className="col-12 col-lg-3 col-md-12 col-sm-12 col-xs-12">
        <LazyLoadImage
          src={`/${AppImage.base}${props.phase.Icon}`}
          alt={props.phase.Title}
          className="phase-icon"
          style={{ width: '100%', maxWidth: '250px' }}
        />
        <h3 className="phase-title">{props.phase.Title}</h3>
        <PositiveButton icon="info" iconPosition="left" additionalClass="mt-2em mb-2em" disabled={disabled} onClick={openDetailPane}>
      ...
      <div className="col-12 col-lg-9 col-md-12 col-sm-12 col-xs-12">
        {(props.phase.Milestones ?? []).map((milestone: ExpeditionSeasonMilestone, index: number) => (
          <ExpeditionSeasonPhaseMilestone key={`${milestone?.Id}-${index}`} milestone={milestone} setDetailPane={props.setDetailPane} />
        ))}
      </div>
    </div>
  );
...
export const ExpeditionSeasonPhaseMilestone: React.FC<IExpeditionSeasonPhaseMilestonesProps> = (props: IExpeditionSeasonPhaseMilestonesProps) => {
  ...
  const classes = classNames('expedition-season-milestone noselect', {
  ...
  return (
    <div data-id="ExpeditionSeasonPhaseMilestone" className={classes}>
      <div className="milestone-content" onClick={openDetailPane}>
        ...
        {props.milestone.Type === MilestoneType.Optional && <div className="middle-ribbon">Optional</div>}
      </div>
      {isEncrypted && (
        <div className="milestone-encr-content noselect ">
          <ImageContainer Icon={props.milestone.Encryption.Icon} Name={props.milestone?.Encryption?.Title ?? 'encrypted'} />
