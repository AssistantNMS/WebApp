...
export const TwitchDropViewerPageUnconnected: React.FC<IProps> = (props: IProps) => {
  ...
  useEffect(() => {
    ...
    const twitchId = url.substring(seasIdSlashIndex + 1, url.length);
    if (isNaN(Number(twitchId))) return;
  ...
  const displayTwitchDropDay = (startDate: Date, drop: TwitchDropDay) => {
    ...
    return (
      <div key={drop.DayNumber} className="gen-item col-12">
        ...
        <GenericListPresenter list={rewards} presenter={RequiredItemListTile} isCentered={shouldListBeCentered(rewards.length)} limitResultsTo={12} />
        <br />
        <hr />
      </div>
    );
    ...
    return (
      <div key={drop.Id} className="row pb1" data-key={drop.Id}>
        <div className="gen-item col-12 pt1">
          <h4>
            <b>{translate(LocaleKey.startDate)}</b>: {twitchDate(drop.StartDate)}
          </h4>
          <h4>
            <b>{translate(LocaleKey.endDate)}</b>: {twitchDate(drop.EndDate)}
          </h4>
        </div>
        ...
        <div className="gen-item col-12">
          <div className="row">{drop.Days.map((day) => displayTwitchDropDay(drop.StartDate, day))}</div>
        </div>
      </div>
    );
  };
    ...
    <DefaultAnimation>
      ...
      <div data-id="TwitchDropViewerPage" className="content">
        <div className="row full pt1 pb5">
          <div className="col-12">
            <div className="generic-item-list">{displayTwitchDrop(twitchDropItem)}</div>
          </div>
        </div>
      </div>
    </DefaultAnimation>
...
export const TwitchDropViewerPage = withServices<IWithoutDepInj, IWithDepInj>(TwitchDropViewerPageUnconnected, (services: IDependencyInjection) => ({
