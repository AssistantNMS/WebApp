...
export function ContributorComponent<T>(props: IProps<T>) {
  ...
  const displayContributors = (communityLinks: Array<T>) => {
    ...
    return <GenericListPresenter list={communityLinks} presenter={props.renderer} identifier={props.identifier} />;
  };

  return (
    <DefaultAnimation>
      <div className="row full pb5">
        <div className="col-12 community-search-list">{displayContributors(contributors)}</div>
      </div>
    </DefaultAnimation>
  );
}
