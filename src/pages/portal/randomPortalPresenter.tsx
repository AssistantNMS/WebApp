...
export const RandomPortalUnconnected: React.FC<IProps> = (props: IProps) => {
  ...
  const portalList = Array.from(Array(16).keys()).map((_: unknown, index: number) => index.toString(16));
        ...
        <div key={`slot-${uuid}-${index}`} className="portal-slot col-2 col-xl-2 col-lg-2 col-md-2 col-sm-3">
          <div className="portal-slot-wheel noselect">
            <div className="wheel-content">
              <img src="/assets/images/portals/dot.png" alt="portal-spacer" className="portal-slot-item-placeholder" />
              <div ref={slotRefs[index]} className="disc">
                {localList.map((portalCode: string, index: number) => (
                  <div key={`${portalCode}-${index}`} data-id={portalCode.toUpperCase()} className={`portal-slot-item item-${index}`}>
  ...
  return (
    <DefaultAnimation>
      ...
      <div className="content">
        ...
        <div className="row full pt1 slots">{displaySlots()}</div>
      </div>
