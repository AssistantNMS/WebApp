    ...
    return (
      <div className="row justify" style={{ paddingLeft: '1em', paddingRight: '1em' }}>
        {(tags || []).map((item: string, index: number) => {
          return (
            <span key={`portalTag: ${item}-${index}`} onClick={() => props.removeTag(item)} className="secondary chip extra-padding pointer">
              {item}
            </span>
          );
        })}
        <span key={`portalTag-add`} onClick={props.addTag} className="secondary chip extra-padding pointer">
          +
        </span>
      </div>
    );
    ...
    <DefaultAnimation>
      ...
      <div className="content">
        <div className="row full pt1">
          <div className="col-12">
            <PortalGlyphGridDisplay codes={props.record.Codes || []} useAltGlyphs={props.useAltGlyphs} />
          </div>
        </div>
        ...
        <div className="row full">
          <div className="col-12">
            <PortalGlyphKeypadGrid useAltGlyphs={props.useAltGlyphs} onPortalClick={props.addCode} />
          </div>
        </div>
      ...
      {props.record != null && props.record.Codes != null && props.record.Codes.length === 12
        ? ApplyFloatingActionButton('portal-add-edit', () => {
            const changeFunc = props.isEdit ? props.editPortal : props.addPortal;
            if (changeFunc != null) {
