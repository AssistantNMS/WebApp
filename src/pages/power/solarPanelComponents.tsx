...
const headingText = (locale: LocaleKey) => <span className="table-heading">{translate(locale)}</span>;

const rowText = (text: string) => <p className="table-text">{text}</p>;

const rowNumeric = (value: number, langCode: string) => rowText(localizeNumber(roundDecimalNum(value, 0), langCode));

const correctlySizedImageFromIcon = (icon: string) => (
  <div className="table-icon">
    <i key={icon} className="material-icons">
      {icon}
    </i>
  </div>
);
...
export const InGameInfoTable: React.FC = () => {
  return (
    <div className="solar-calc card mt-0">
      <table>
        <tbody>
...
export const RealTimeInfoTable: React.FC = () => {
  return (
    <div className="solar-calc card mt-0">
      <table>
        <tbody>
...
export const minimumPanelsAndBatteries = (solarPanelReqItem: RequiredItemDetails, batteryReqItem: RequiredItemDetails) => {
  return (
    <>
...
export const recommendedPanelsAndBatteries = (
  ...
  const resArray: Array<RequiredItemDetails> = [
    ...
    {
      ...batteryReqItem,
      Quantity: getRecommendedAmountOfBatteries(minBatteries, totalPowerLost),
    },
  ];
  return (
    <>
      ...
      <div className="col-12">
        <GenericListPresenter list={resArray} presenter={RequiredItemDetailsListTile} isCentered={true} limitResultsTo={3} />
      </div>
    </>
  );
};
