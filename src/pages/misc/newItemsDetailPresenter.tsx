...
export const NewItemsDetailPresenterUnconnected: React.FC<IProps> = (props: IProps) => {
  ...
  useEffect(() => {
    ...
    const updateId = url.substring(idSlashIndex + 1, url.length);
  ...
  const loadUpdateItems = async (updateId: string) => {
    ...
    setGameDetails(detailsResult.value.sort(gameItemModelSortByName));
  ...
  const handleLoadingOrError = () => {
    ...
    return (
      <>
        <ItemHeaderRow Icon={update.icon} Name={update.title} Abbrev={update.gameVersion} Group={update.emoji}>
          <p>
            {formatDate(update.releaseDate, 'YYYY-MM-DD')}
            <br />
        ...
        <div className="mt-1em pb5">
          <GenericListPresenter list={gameDetails} presenter={(props: GameItemModel): JSX.Element => <GenericItemListTile {...props} />} />
        </div>
      </>
    );
  };
  ...
  return (
    <DefaultAnimation>
      ...
      <div className="content new-item">{handleLoadingOrError()}</div>
    </DefaultAnimation>
  );
};
