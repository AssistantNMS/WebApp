...
export function GenericListPresenter<T>(props: IProps<T>) {
    ...
    if (showMore) {
      ...
      return (
        <div className="col-12 mt-1em">
          <PositiveButton onClick={() => setShowMore(false)}>Show Less</PositiveButton>
        </div>
      );
    } else {
      ...
      return (
        <div className="col-12 mt-1em">
          <PositiveButton onClick={() => setShowMore(true)}>{translate(LocaleKey.viewXMore).replace('{0}', xRecordsLeft)}</PositiveButton>
        </div>
      );
    }
  };
  ...
  return (
    <div
      className={classNames('generic-item-list row', {
        justify: props.isCentered,
      })}
    >
