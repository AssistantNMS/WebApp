...
export const DecriptionRegexHighlightText: React.FC<IProps> = (props: IProps) => {
  ...
  const groupMatches = props.orig.includes('<>');
  ...
  const renderNode = (paragraphIndex: number, wordIndex: number, word: string, colour: string, wordChain: string) => (
    <span key={`paragraph-${paragraphIndex}-word-${wordIndex}-${word}`} style={{ color: colour }}>
      {wordChain}
    </span>
  );
        ...
        if (localTag === '<IMG>') {
          ...
          nodes.push(
            <CustomTooltip
              ...
              tooltipText="Not the right platform? Change your platform at the top of the page or in the settings page!"
              theme="transparent"
            >
              <img className="descrip-img" src={`/${AppImage.controls}${lookupResIcon}`} alt={lookupResKey} data-lookup-key={lookupKey} />
            </CustomTooltip>,
          );
        } else {
          nodes.push(
            renderNode(paragraphIndex, wordIndex + 999, displayWord, currentColourValue, displayWord + (leftOverDisplayWord.length > 0 ? '' : ' ')),
          );
        }
