...
export const RequiredItemsQuantityContainer: React.FC<IRequiredItemQuantityProps> = (props: IRequiredItemQuantityProps) => {
  ...
  for (let inputIndex = startIndex; inputIndex < props.requiredItems.length; inputIndex++) {
    ...
    const localEndIndex = inputIndex === (props.limitRequiredItems ?? 0) - 1 ? inputIndex : endIndex;
  ...
  if (addEtc) {
    ...
    quantities.push(
      <span key="quantity-etc">
        {props.requiredItems.length - (props.limitRequiredItems ?? endIndex)} {translate(LocaleKey.more)}...
      </span>,
    );
  }

  return <BaseTooltipQuantityContainer tooltipText={subtitle} spans={quantities} />;
};
