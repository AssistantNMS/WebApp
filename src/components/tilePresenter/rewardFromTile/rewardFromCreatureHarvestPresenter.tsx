...
export const RewardFromCreatureHarvestTile: React.FC<IProps> = (props: IProps) => {
    ...
    if (description != null && description.length > 0) {
        const localeKey = props.creatureHarvest.HarvestType === 0
            ...
            <div className="gen-item-content-container" data-id="RewardFromCreatureHarvestTile">
                <TextContainer text={props.creatureHarvest.CreatureType} additionalCss={displayTextFull ? 'full' : ''} />
                {
                    displayTextFull
                        ...
                        : <>{
                            (wikiLinkInDescrip === true)
                                ? (<div className="quantity-container">{translate(LocaleKey.viewMoreOnNmsWiki)}&nbsp;<i className="material-icons">open_in_new</i></div>)
                                : (<div className="quantity-container">{description}</div>)
                        }</>
                }
                <ActionContainer actions={displayLinkOnTheRightHandside ? [
                    <div className="max-w-150">
                        {translate(LocaleKey.viewMoreOnNmsWiki)}
                        <br /><i className="material-icons">open_in_new</i>
                    </div>
                ] : []} />
            </div>
        </>
    ...
    if (hasWikiLink) {
        return (
            <BasicLink key={`creature-harvest-${props.creatureHarvest.ItemId}`} additionalClassNames="gen-item-container" href={hasWikiLink ? props.creatureHarvest.WikiLink : '#'}>
                {content}
            </BasicLink>
        );
    }
