...
const QuicksilverItemListTileClass: React.FC<IQuicksilverItemProps> = (props: IQuicksilverItemProps) => {
  ...
  const getActions = () => {
    ...
    if (props.isDisabled) {
      result.push(
        <i key="locked" className="material-icons x2 not-centered">
          lock
        </i>,
      );
    }
  ...
  return (
    <Link to={`${catalogueItem}/${props.ItemId}`} data-id="QuicksilverItemListTile" className="gen-item-container" draggable={false}>
      ...
      <div className="gen-item-content-container">
        ...
        {props.Tier != null && props.Tier > 0 && <div className="quantity-container">Tier: {props.Tier}</div>}
...
export const QuicksilverItemListTile = (props: IQuicksilverItemWithoutDepInj): JSX.Element => <QuicksilverItemListTileWithDepInj {...props} />;
