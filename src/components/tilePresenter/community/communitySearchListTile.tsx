...
export const CommunitySearchListTile =
  (chipColours: Array<CommunitySearchChipColourViewModel>, setSelectedItem: (item: CommunitySearchViewModel) => void) =>
  (props: CommunitySearchViewModel, index: number) => {
    return (
      <div
        key={props.links[0] + ' ' + index}
        data-id="CommunitySearchListTile"
        className="community-search-card-bg noselect"
        onClick={() => setSelectedItem(props)}
      >
        <div className="community-search-card pointer">
          <div className="community-search-top">
            <div className="community-icon">
              <LazyLoadImage src={`${ExternalUrls.communitySearch}${props.icon}`} alt={props.name ?? props.icon} draggable={false} />
            </div>
          ...
          <div className="community-search-tags">{communitySearchTags(chipColours, props.tags)}</div>
        </div>
      </div>
    );
  };
