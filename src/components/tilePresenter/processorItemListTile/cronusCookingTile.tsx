...
const CronusCookingListTileClass: React.FC<ICronusCookingProps> = (props: ICronusCookingProps) => {
    ...
    const cookingPerc: string =
        ((props.cookingValue + 1) * props.cookingValue * 47).toFixed(0);
    ...
    const getStars = (): ReactNode => {
        ...
        for (let starValIndx = 0; starValIndx < 5; starValIndx++) {
            if (starValue > 0.75) {
                starNodes.push(<i key={starValIndx + 'full'} className="material-icons secondary">star</i>);
                starValue = starValue - 1;
                continue;
            }

            if (starValue > 0.25) {
                starNodes.push(<i key={starValIndx + 'half'} className="material-icons secondary">star_half</i>);
                starValue = starValue - 1;
                continue;
            }

            starNodes.push(<i key={starValIndx + 'empty'} style={{ opacity: 0.5 }} className="material-icons">star</i>);
            starValue = starValue - 1;
        }
