...
import {
  ...
  getMenuSection3,
  getMenuSection4,
  getMenuSection4Async,
  getMenuSection5,
  menuItemSeperator,
} from '../../../helper/drawerMenuItemsHelper';
...
const DrawerUnconnected: React.FC<IProps> = (props: IProps) => {
  ...
  const getMenuItems = async () => {
    ...
    const localMenuItems = [...getMenuSection1(), ...getMenuSection2(), ...getMenuSection3(), ...menuSection4Items, ...getMenuSection5()];
  ...
  const renderMenuItemChild = (menuItem: DrawerMenuItem, icon: JSX.Element | undefined, isActive: boolean): ReactNode => {
    if (menuItem.link.includes('http')) {
      return (
        <a onClick={menuItemClick} href={menuItem.link} target="_blank" rel="noopener noreferrer" className="nav-link">
          {icon}
          <p>{menuItem.name}</p>
        </a>
      );
    ...
    return (
      <a onClick={menuItemClick} href="#" className="nav-link" draggable={false}>
        {icon}
        <p>{menuItem.name}</p>
      </a>
    );
  ...
  const renderMenuItem = (pathname: string) => (menuItem: DrawerMenuItem, index: number) => {
    ...
    if (menuItem.iconType === DrawerIconType.Material)
      icon = (
        <i key={menuItem.icon} className="material-icons">
          {menuItem.icon}
        </i>
      );
    if (menuItem.iconType === DrawerIconType.Custom)
      icon = <img key={menuItem.icon} className="custom-icons" src={menuItem.icon} alt={menuItem.icon} />;
    ...
    return (
      <li key={`${menuItem.link}-${index}`} className={classes} draggable={false}>
        {child}
        {(menuItem?.subs?.length ?? 0) > 0 && (
          <>
            <i className={subMenuIconClasses} onClick={toggleSubMenu(menuItem.icon)}>
              expand_more
            </i>
            <ul
              className={classNames('nav', 'sub', {
                expanded: subMenuIsExpanded,
              })}
            >
  ...
  const renderMenuItems = (menuItems: Array<DrawerMenuItem>) => menuItems.map(renderMenuItem(location.pathname));

  return (
    <div key={`drawer-menu-${props.selectedLanguage}`} className={classNames('sidebar noselect', props.selectedLanguage)}>
      <div className="sidebar-wrapper ps-theme-default">
        <ul className="nav">
          <div className="logo">
            <Link to="/" draggable={false}>
              <img src="/assets/images/DrawerHeader.png" draggable={false} alt="drawerHeader" />
            </Link>
          </div>
