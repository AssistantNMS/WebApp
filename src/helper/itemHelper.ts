...
export const getAllRequiredItemsForMultiple = async (
  gameItemService: GameItemService,
  requiredItems: Array<RequiredItem>,
): Promise<Array<RequiredItemDetails>> => {
  const rawMaterials: Array<RequiredItemDetails> = [];
  for (const requiredItem of requiredItems) {
    const tempItems = await getRequiredItems(gameItemService, requiredItem);
    for (let tempItemIndex = 0; tempItemIndex < tempItems.length; tempItemIndex++) {
  ...
  for (let rawMaterialIndex = 0; rawMaterialIndex < rawMaterials.length; rawMaterialIndex++) {
...
export const getRequiredItems = async (gameItemService: GameItemService, requiredItem: RequiredItem): Promise<Array<RequiredItemDetails>> => {
  ...
  const requiredItemDetails: RequiredItemDetails = toRequiredItemDetails(requiredItem, genericResult.value);

  const rawMaterialsResult: Array<RequiredItemDetails> = [];

  if (tempRawMaterials != null && tempRawMaterials.length === 0 && requiredItemDetails != null) {
    rawMaterialsResult.push(requiredItemDetails);
    return rawMaterialsResult;
  }

  for (let requiredIndex = 0; requiredIndex < tempRawMaterials.length; requiredIndex++) {
...
export const requiredItemDetailsFromInputs = async (
  gameItemService: GameItemService,
  inputs: Array<RequiredItem>,
): Promise<ResultWithValue<Array<RequiredItemDetails>>> => {
...
export const getRequiredItemsForTreeItem = async (
  gameItemService: GameItemService,
  requiredItem: RequiredItem,
): Promise<Tree<RequiredItemDetails> | null> => {
  const genericResult = await gameItemService.getItemDetails(requiredItem.Id);
  if (!genericResult.isSuccess) {
    console.error(`genericItemResult hasFailed: ${genericResult.errorMessage}`);
    return null;
  ...
  const currentItemDetail: RequiredItemDetails = toRequiredItemDetails(requiredItem, genericResult.value);
...
export const getAllRequiredItemsForTree = async (
  gameItemService: GameItemService,
  requiredItems: Array<RequiredItem>,
): Promise<Array<Tree<RequiredItemDetails>>> => {
